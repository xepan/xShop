...
class page_xShop_page_designer_save extends Page {
	function page_index(){
		...
		}elseif(isset($target) and ($_POST['designer_mode']=='false' OR !isset($_POST['designer_mode'])) and $target['member_id'] == $designer->id ){
		...
		}elseif(isset($target) and ($_POST['designer_mode']=='false' OR !isset($_POST['designer_mode']))){
			$target = $this->add('xShop/Model_ItemMemberDesign');
			$target->addCondition('item_id', $_POST['item_id']);
